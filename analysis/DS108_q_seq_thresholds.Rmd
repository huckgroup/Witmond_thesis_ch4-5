---
title: "DS108 ID-seq analysis: Fold change thresholds"
author: "mwitmond"
date: "2024-02-26"
output:
...
cts <- counts_dtbl  %>%
  dplyr::filter(stimulus == "PBS" & inhib_conc == "0 uM (PBS)" & true_counts >= 1) %>%
...
for(dataset in c(1:length(list_results_volc))){
  plot <- vulcanoplot_results(data = list_results_volc[[dataset]], 
                              ...
                              xintercepts = c(-log2(1.05), log2(1.05)))
  print(plot)
}
```
...
for(dataset in c(1:length(list_results_volc))){
  assign(names_results_heat[[dataset]], filter_heatmap(dataset = list_results_volc[[dataset]], select_top = FALSE, foldchange_filter = log2(1.05)))
  dataset
}
...
***Figure:** Heatmap of all significant results in the OCI-Ly8 vs HBL1 comparison.*
...
Finally, we visualised the normalised true counts of all significant proteins in the OCI-Ly8 vs HBL1 comparison (7 in total).
...
clean_data_well <- data.frame(data_cell) %>% 
  ...
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_cell) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
...
clean_data_mean <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_cell & stimulus == "PBS" & inhib_conc == "0 uM (PBS)") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
```{r fig_counts_basal_all, fig.width=15, fig.height=12}
...
write(sign_prot_cell, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal/sign_proteins.txt")

# Dataset with significantly changing proteins, and log2FC + pvalues from DESeq2 analysis: clean_data_sign
write.csv(clean_data_sign, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal/sign_DESeq2data.csv", row.names = F)

# Dataset of counts per protein per sample + heatmap data for significantly changing proteins: clean_data_well
write.csv(clean_data_well, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal/sign_sample.csv", row.names = F)

# Dataset of counts per protein per condition (mean of repl) for significantly changing proteins: clean_data_mean
write.csv(clean_data_mean, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal/sign_condition.csv", row.names = F)
```
...
cts <- counts_dtbl  %>%
  dplyr::filter(stimulus == "PBS" & cell_line == "HBL1" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
heat_100uM_H <- list(heat_iSYK100_H, heat_iBTK100_H, heat_iPI3Kd100_H, heat_iNFkB100_H) %>% 
  ...
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
```
***Figure:** Heatmaps of the significant results in the 100 uM inhibitor vs DMSO ctrl comparison.*
...
cts <- counts_dtbl  %>%
  dplyr::filter(stimulus == "PBS" & cell_line == "OCI-Ly8" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
```
***Figure:** Heatmap of all significant results in the 100 uM inhibitor vs DMSO ctrl comparisons.*
...
cts <- counts_dtbl  %>%
  dplyr::filter(stimulus == "PBS" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
```
***Figure:** Heatmap of all significant results in the 100 uM inhibitor vs DMSO comparison and the effect of OCI-Ly8 on 100 uM inhibitor vs DMSO ctrl comparison.*


### Protein comparisons

Finally, we visualised the normalised true counts of all significant proteins in the OCI-Ly8 vs HBL1 comparison (46 total).
...
for (heat_data in c(1:length(heat_list_H))) {
  ...
  alt_data <- heat_list_H[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_100uM_H) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
clean_data_well_100uM <- full_join(alt_data_100uM_H, alt_data_100uM_O) %>%
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_100uM) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
...
clean_data_well_cell_cc <- data.frame(data_cell_inhib_cc) %>% 
  ...
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_cc) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         true_counts, true_scaling_factor_geo, true_counts_norm_geo, counts_norm_0, man_FC_norm, man_log2FC_norm, heatmap_value) %>% # reorder columns for a more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_well <- counts_dtbl %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_all & stimulus == "PBS") %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         true_counts, true_scaling_factor_geo, true_counts_norm_geo, counts_norm_0, man_FC_norm, man_log2FC_norm) %>% # reorder columns for a more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_mean_100uM <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_100uM & stimulus == "PBS") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_mean_cell_cc <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_cc & stimulus == "PBS") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
```{r fig_counts_bas_inhib_all, fig.width=15, fig.height=30}
plot <- ggplot(clean_data_well, 
...
```{r save_bas_inhib}
# List of significantly changing proteins: sign_prot_cell
write(sign_prot_100uM, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/100uM_sign_proteins.txt")
write(sign_prot_cell_cc, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/cell_100uM_sign_proteins.txt")

# Dataset with significantly changing proteins, and log2FC + pvalues from DESeq2 analysis: clean_data_sign
write.csv(clean_data_sign_100uM_H, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/100uM_HBL1_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_100uM_O, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/100uM_Ly8_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_cell_cc, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/cell_100uM_sign_DESeq2data.csv", row.names = F)

# Dataset of counts per protein per sample + heatmap data for significantly changing proteins: clean_data_well
write.csv(clean_data_well_100uM, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/100uM_sign_sample.csv", row.names = F)
write.csv(clean_data_well_cell_cc, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/cell_100uM_sign_sample.csv", row.names = F)

# Dataset of counts per protein per condition (mean of repl) for significantly changing proteins: clean_data_mean
write.csv(clean_data_mean_100uM, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/100uM_sign_condition.csv", row.names = F)
write.csv(clean_data_mean_cell_cc, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/basal_inhibitor/cell_100uM_sign_condition.csv", row.names = F)
```
...
# coldata: dataframe with wells as row names and all metadata as columns
cts <- counts_dtbl  %>%
  dplyr::filter(inhib_conc == "0 uM (PBS)" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
# aIg+H2O2 vs PBS
# heat_cell_stim$marker_pathway[is.na(heat_cell_stim$marker_pathway)] <- "Other"

ann_colors_man <- list(cell_line = colors_cell,
                       stimulus = colors_stim
                       # modification = c(phospho = "#f768a1", no = "grey", cleaved = "#fff7bc"),
                       # marker_pathway = setNames(colors_light12[1:length(unique(heat_cell_stim$marker_pathway))], unique(heat_stim_i$marker_pathway))
)

# Get the rld transformed data for the heatmap and scale per row
data_cell_stim <- t(scale(t(assay(rld)[unique(c(heat_cell_stim$target_nospace)), rownames(data_heatmap)])))
# sort_hclust <- function(...) as.hclust(dendsort(as.dendrogram(...)))
# data_cluster_rows <- sort_hclust(hclust(dist(data_cell)))
# plot(data_cluster_rows, main = "Unsorted Dendrogram", xlab = "", sub = "")
...
ComplexHeatmap::pheatmap(
  ...
  cluster_cols = FALSE,
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  annotation_row = heat_cell_stim[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap$cell_line,
  row_split = subset(heat_cell_stim)$modification,
  cellwidth = 10,
  cellheight = 10,
  fontsize = 8,
  border_color = NA,
...
***Figure:** Heatmap of all significantly different proteins in OCi-Ly8 for the aIg+H2O2 vs PBS comparison.*
...
***Figure:** Heatmap of all significant results in the aIg+H2O2 vs PBS comparison for HBL1 cells.*
...
Finally, we visualised the normalised true counts of all significant proteins in the aIg+H2O2 vs PBS comparison (20 in total).
...
clean_data_well_int <- data.frame(data_stim_i) %>% 
  ...
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_int) %>%
...
clean_data_mean_int <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_int & inhib_conc == "0 uM (PBS)") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_well_sep <- full_join(heat_data_HBL1, heat_data_Ly8) %>% 
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_sep) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, cell_stim, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
...
clean_data_mean_sep <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_cell_sep & inhib_conc == "0 uM (PBS)") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
```{r fig_counts_act_all, fig.width=15, fig.height=12}
...
```{r save_act}
# List of significantly changing proteins: sign_prot_cell
write(sign_prot_cell_int, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/int_sign_proteins.txt")
write(sign_prot_cell_sep, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/sep_sign_proteins.txt")

# Dataset with significantly changing proteins, and log2FC + pvalues from DESeq2 analysis: clean_data_sign
write.csv(clean_data_sign_int, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/int_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_sep, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/sep_sign_DESeq2data.csv", row.names = F)

# Dataset of counts per protein per sample + heatmap data for significantly changing proteins: clean_data_well
write.csv(clean_data_well_int, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/int_sign_sample.csv", row.names = F)
write.csv(clean_data_well_sep, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/sep_sign_sample.csv", row.names = F)

# Dataset of counts per protein per condition (mean of repl) for significantly changing proteins: clean_data_mean
write.csv(clean_data_mean_int, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/int_sign_condition.csv", row.names = F)
write.csv(clean_data_mean_sep, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active/sep_sign_condition.csv", row.names = F)
```
...
cts <- counts_dtbl  %>%
  dplyr::filter(cell_line == "HBL1" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
data_heatmap_short <- as.data.frame(colData(dds)[, c("stimulus", "inhib_new", "inhib_conc")]) %>%
  dplyr::filter(inhib_new != "PBS ctrl" & !(stimulus == "PBS" & inhib_new %in% inhib_DS108)) %>%
  arrange(factor(stimulus, levels = stim_DS108), factor(inhib_new, levels = inhib_new_DS108), factor(inhib_conc, levels = inhib_conc_DS108))
```
...
```{r fig_heatmap_act_HBL1_int_100s, fig.width=15, fig.height=5.5}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)
...
# 100 uM inhibitor effect on aIg+H2O2 vs PBS (short)
# Get the rld transformed data for the heatmap and scale per row
data_100_stim_Hi <- t(scale(t(assay(rld)[unique(c(heat_100_stim_Hi$target_nospace)), rownames(data_heatmap_short)])))
# sort_hclust <- function(...) as.hclust(dendsort(as.dendrogram(...)))
# data_cluster_rows <- sort_hclust(hclust(dist(data_cell)))
# plot(data_cluster_rows, main = "Unsorted Dendrogram", xlab = "", sub = "")
...
as.ggplot(ComplexHeatmap::pheatmap(
  ...
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  # annotation_row = heat_cell[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap_short,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_short$inhib_new,
  row_split = subset(heat_100_stim_Hi)$modification,
  cellwidth = 10,
  cellheight = 10,
  fontsize = 9,
...
***Figure:** Heatmaps of the significant results in the 100 uM inhibitor vs DMSO ctrl comparisons and the effect of 100 uM inhibitor on aIg+H2O2 vs PBS comparisons.*
...
results_iBTK0.1_Hcs <- results(dds, name = "inhib_text_new0.1.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK1_Hcs <- results(dds, name = "inhib_text_new1.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK10_Hcs <- results(dds, name = "inhib_text_new10.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK100_Hcs <- results(dds, name = "inhib_text_new100.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)

results_iPI3Kd0.1_Hcs <- results(dds, name = "inhib_text_new0.1.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd1_Hcs <- results(dds, name = "inhib_text_new1.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd10_Hcs <- results(dds, name = "inhib_text_new10.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd100_Hcs <- results(dds, name = "inhib_text_new100.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
...
```{r fig_log2FC_act_HBL1_conc, fig.width=7, fig.height=4}
par(mfrow = c(1,3), mar = c(4,4,2,1))
# par(mfrow = c(1,1))
signcutoff <- 0.05
plotMA(results_iBTK1_Hcs, alpha = signcutoff, main = "Effect of 1 uM iBTK on\n aIg+H2O2 vs PBS")
plotMA(results_iBTK10_Hcs, alpha = signcutoff, main = "Effect of 10 uM iBTK on\n aIg+H2O2 vs PBS")
plotMA(results_iBTK100_Hcs, alpha = signcutoff, main = "Effect of 100 uM iBTK on\n aIg+H2O2 vs PBS")
```


##### Data transformation {.tabset}

```{r transformation_act_HBL1_conc}
# Normtransform (log2(n+1))
ntd <- normTransform(dds)
...
```{r fig_PCA_act_HBL1_conc, fig.width=10, fig.height=4}
list_datatransform <- list(ntd, vsd, rld)
titles_datatransform <- c("ntd", "vsd", "rld")
...
```{r fig_sd_act_HBL1_conc, fig.width=6, fig.height=3}
options(repr.plot.width = 6, repr.plot.height = 3)
...
```{r fig_distances_act_HBL1_conc, fig.width=10, fig.height=15}
options(repr.plot.width = 10, repr.plot.height = 15)
...
```{r fig_volcano_act_HBL1_conc, fig.width=8, fig.height=6.5}
# Function to create volcano plots
# Standard log2FC threshold = log2(1.5); standard padj threshold = 0.05
vulcanoplot_results <- function(data = data, 
                                title_plot = "", 
...
# Apply function to each dataset
list_results_volc <- list(volc_iSYK0.1_Hcs, volc_iSYK1_Hcs, volc_iSYK10_Hcs, volc_iSYK100_Hcs, 
                        volc_iBTK0.1_Hcs, volc_iBTK1_Hcs, volc_iBTK10_Hcs, volc_iBTK100_Hcs, 
                        volc_iPI3Kd0.1_Hcs, volc_iPI3Kd1_Hcs, volc_iPI3Kd10_Hcs, volc_iPI3Kd100_Hcs, 
                        volc_iNFkB0.1_Hcs, volc_iNFkB1_Hcs, volc_iNFkB10_Hcs, volc_iNFkB100_Hcs)
titles_volcano <- c("Effect of 0.1 uM iSYK on aIg+H2O2 vs PBS (HBL1)", "Effect of 1 uM iSYK on aIg+H2O2 vs PBS (HBL1)", "Effect of 10 uM iSYK on aIg+H2O2 vs PBS (HBL1)", "Effect of 100 uM iSYK on aIg+H2O2 vs PBS (HBL1)", 
                    "Effect of 0.1 uM iBTK on aIg+H2O2 vs PBS (HBL1)", "Effect of 1 uM iBTK on aIg+H2O2 vs PBS (HBL1)", "Effect of 10 uM iBTK on aIg+H2O2 vs PBS (HBL1)", "Effect of 100 uM iBTK on aIg+H2O2 vs PBS (HBL1)", 
                    "Effect of 0.1 uM iPI3Kd on aIg+H2O2 vs PBS (HBL1)", "Effect of 1 uM iPI3Kd on aIg+H2O2 vs PBS (HBL1)", "Effect of 10 uM iPI3Kd on aIg+H2O2 vs PBS (HBL1)", "Effect of 100 uM iPI3Kd on aIg+H2O2 vs PBS (HBL1)", 
                    "Effect of 0.1 uM iNFkB on aIg+H2O2 vs PBS (HBL1)", "Effect of 1 uM iNFkB on aIg+H2O2 vs PBS (HBL1)", "Effect of 10 uM iNFkB on aIg+H2O2 vs PBS (HBL1)", "Effect of 100 uM iNFkB on aIg+H2O2 vs PBS (HBL1)")

for(dataset in c(1:length(list_results_volc))){
  plot <- vulcanoplot_results(data = list_results_volc[[dataset]], 
                              title_plot = titles_volcano[dataset], 
                              xintercepts = c(-log2(1.1), log2(1.1))) + 
    coord_cartesian(xlim = c(-1.5, 0.5), ylim = c(0, 100))
  print(plot)
}
```

***Figure:** Volcano plots of DESeq2 model comparisons: the effect of inhibitor conc on aIg+H2O2 vs PBS (HBL1 cells only). Higher conc of inhibitor significantly affects more proteins, except iPI3Kd.*
...
# Apply function to each dataset
names_results_heat <- c("heat_iSYK0.1_Hcs", "heat_iSYK1_Hcs", "heat_iSYK10_Hcs", "heat_iSYK100_Hcs", 
                        "heat_iBTK0.1_Hcs", "heat_iBTK1_Hcs", "heat_iBTK10_Hcs", "heat_iBTK100_Hcs", 
                        "heat_iPI3Kd0.1_Hcs", "heat_iPI3Kd1_Hcs", "heat_iPI3Kd10_Hcs", "heat_iPI3Kd100_Hcs", 
                        "heat_iNFkB0.1_Hcs", "heat_iNFkB1_Hcs", "heat_iNFkB10_Hcs", "heat_iNFkB100_Hcs")
...
```{r fig_heatmap_act_HBL1_conc_iSYK, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iSYK effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iSYK_Hcs <- list(heat_iSYK0.1_Hcs, heat_iSYK1_Hcs, heat_iSYK10_Hcs, heat_iSYK100_Hcs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# Subset the full dataframe to get metadata for heatmap plotting
data_heatmap_iSYK <- as.data.frame(colData(dds)[, c("stimulus", "inhibitor", "inhib_new", "inhib_conc")]) %>%
  dplyr::filter(inhibitor == "iSYK" & inhib_new != "PBS ctrl") %>%
  dplyr::select(-inhibitor) %>%
  arrange(factor(stimulus, levels = stim_DS108), factor(inhib_new, levels = inhib_new_DS108), factor(inhib_conc, levels = inhib_conc_DS108))

# Annotation colors of the heatmap
ann_colors_man <- list(inhib_new = c("PBS ctrl" = "black", "DMSO ctrl" = "grey", colors_inhib),
                       stimulus = colors_stim, 
                       inhib_conc = setNames(colors_blue9[2:(length(unique(data_heatmap$inhib_conc))+1)], unique(data_heatmap$inhib_conc))
)

# 100 uM iSYK effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iSYK_Hcs <- t(scale(t(assay(rld)[unique(c(heat_iSYK_Hcs$target_nospace)), rownames(data_heatmap_iSYK)])))
...
ComplexHeatmap::pheatmap(
  ...
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  # annotation_row = heat_cell[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap_iSYK,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iSYK$inhib_new,
  # row_split = subset(heat_iSYK_Hcs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iSYK conc on aIg+H2O2 vs PBS (HBL1)"
)
```

```{r fig_heatmap_act_HBL1_conc_iBTK, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iBTK effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iBTK_Hcs <- list(heat_iBTK0.1_Hcs, heat_iBTK1_Hcs, heat_iBTK10_Hcs, heat_iBTK100_Hcs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# Subset the full dataframe to get metadata for heatmap plotting
data_heatmap_iBTK <- as.data.frame(colData(dds)[, c("stimulus", "inhibitor", "inhib_new", "inhib_conc")]) %>%
  dplyr::filter(inhibitor == "iBTK" & inhib_new != "PBS ctrl") %>%
  dplyr::select(-inhibitor) %>%
  arrange(factor(stimulus, levels = stim_DS108), factor(inhib_new, levels = inhib_new_DS108), factor(inhib_conc, levels = inhib_conc_DS108))
...
# 100 uM iBTK effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iBTK_Hcs <- t(scale(t(assay(rld)[unique(c(heat_iBTK_Hcs$target_nospace)), rownames(data_heatmap_iBTK)])))
...
ComplexHeatmap::pheatmap(
  ...
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  # annotation_row = heat_cell[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap_iBTK,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iBTK$inhib_new,
  # row_split = subset(heat_iBTK_Hcs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iBTK conc on aIg+H2O2 vs PBS (HBL1)"
)
```

```{r fig_heatmap_act_HBL1_conc_iPI3Kd, fig.width=8, fig.height=4, eval=F}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iPI3Kd effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iPI3Kd_Hcs <- list(heat_iPI3Kd0.1_Hcs, heat_iPI3Kd1_Hcs, heat_iPI3Kd10_Hcs, heat_iPI3Kd100_Hcs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# Subset the full dataframe to get metadata for heatmap plotting
data_heatmap_iPI3Kd <- as.data.frame(colData(dds)[, c("stimulus", "inhibitor", "inhib_new", "inhib_conc")]) %>%
  dplyr::filter(inhibitor == "iPI3Kd" & inhib_new != "PBS ctrl") %>%
  dplyr::select(-inhibitor) %>%
  arrange(factor(stimulus, levels = stim_DS108), factor(inhib_new, levels = inhib_new_DS108), factor(inhib_conc, levels = inhib_conc_DS108))
...
# 100 uM iPI3Kd effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iPI3Kd_Hcs <- t(scale(t(assay(rld)[unique(c(heat_iPI3Kd_Hcs$target_nospace)), rownames(data_heatmap_iPI3Kd)])))
...
as.ggplot(ComplexHeatmap::pheatmap(
  ...
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  # annotation_row = heat_cell[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap_iPI3Kd,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iPI3Kd$inhib_new,
  # row_split = subset(heat_iPI3Kd_Hcs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  ...
  labs(title = "Effect of iPI3Kd conc on aIg+H2O2 vs PBS (HBL1)")
```

```{r fig_heatmap_act_HBL1_conc_iNFkB, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iNFkB effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iNFkB_Hcs <- list(heat_iNFkB0.1_Hcs, heat_iNFkB1_Hcs, heat_iNFkB10_Hcs, heat_iNFkB100_Hcs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# Subset the full dataframe to get metadata for heatmap plotting
data_heatmap_iNFkB <- as.data.frame(colData(dds)[, c("stimulus", "inhibitor", "inhib_new", "inhib_conc")]) %>%
  dplyr::filter(inhibitor == "iNFkB" & inhib_new != "PBS ctrl") %>%
  dplyr::select(-inhibitor) %>%
  arrange(factor(stimulus, levels = stim_DS108), factor(inhib_new, levels = inhib_new_DS108), factor(inhib_conc, levels = inhib_conc_DS108))
...
# 100 uM iNFkB effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iNFkB_Hcs <- t(scale(t(assay(rld)[unique(c(heat_iNFkB_Hcs$target_nospace)), rownames(data_heatmap_iNFkB)])))
...
ComplexHeatmap::pheatmap(
  ...
  # annotation_row = heat_cell[, c("modification", "marker_pathway")],
  # annotation_row = heat_cell[, "marker_pathway", drop = FALSE],
  annotation_col = data_heatmap_iNFkB,
  annotation_colors = ann_colors_man,
  # scale = "row",
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iNFkB$inhib_new,
  # row_split = subset(heat_iNFkB_Hcs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iNFkB conc on aIg+H2O2 vs PBS (HBL1)"
)
```

***Figure:** Heatmaps of the significant results in the effect of inhibitor conc on aIg+H2O2 vs PBS comparisons.*
...
cts <- counts_dtbl  %>%
  dplyr::filter(cell_line == "HBL1" & stimulus == "aIg+H2O2" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
***Figure:** Heatmap of the significant results in the 100 uM inhibitor vs DMSO ctrl comparisons.*



### HBL1: Protein comparisons

Finally, we visualised the normalised true counts of all proteins for which the aIg+H2O2 vs PBS comparison was significantly affected by 100 uM inhibitor (23 in total).
...
sign_prot_iSYK_Hcs <- sort(unique(heat_iSYK_Hcs$target_nospace))
sign_prot_iBTK_Hcs <- sort(unique(heat_iBTK_Hcs$target_nospace))
sign_prot_iNFkB_Hcs <- sort(unique(heat_iNFkB_Hcs$target_nospace))
...
for (heat_data in c(1:length(heat_list_Hi))) {
  ...
  alt_data <- heat_list_Hi[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_Hi) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iSYK_Hs))) {
  ...
  alt_data <- heat_list_iSYK_Hs[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iSYK_Hs) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iBTK_Hs))) {
  ...
  alt_data <- heat_list_iBTK_Hs[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iBTK_Hs) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iPI3Kd_Hs))) {
  ...
  alt_data <- heat_list_iPI3Kd_Hs[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iPI3Kd_Hs) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iNFkB_Hs))) {
  ...
  alt_data <- heat_list_iNFkB_Hs[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iNFkB_Hs) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
data_iSYK_Hs <- t(scale(t(assay(rld)[unique(c(sign_prot_iSYK_Hs)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iSYK")))])))
...
data_iBTK_Hs <- t(scale(t(assay(rld)[unique(c(sign_prot_iBTK_Hs)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iBTK")))])))
...
data_iPI3Kd_Hs <- t(scale(t(assay(rld)[unique(c(sign_prot_iPI3Kd_Hs)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iPI3Kd")))])))
...
data_iNFkB_Hs <- t(scale(t(assay(rld)[unique(c(sign_prot_iNFkB_Hs)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iNFkB")))])))
...
clean_data_well_inhib_Hs <- list(alt_data_iSYK_Hs, alt_data_iBTK_Hs, alt_data_iPI3Kd_Hs, alt_data_iNFkB_Hs) %>% 
  ...
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_inhib_Hs) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
...
clean_data_mean_Hi <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_Hi & cell_line == "HBL1") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_mean_Hs <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_inhib_Hs & cell_line == "HBL1") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
```{r fig_counts_act_HBL1_all, fig.width=15, fig.height=15}
...
Effect of inhibitor concentration on aIg+H2O2 vs PBS comparison (HBL1 cells only):
For each inhibitor (iSYK, iBTK, iPI3Kd, iNFkB):

- HBL1_inhib_stim_sign_proteins.txt: List of significantly affected proteins by any inhibitor concentration in the aIg+H2O2 vs PBS comparison
...
```{r save_act_HBL1}
# List of significantly changing proteins: sign_prot_cell
write(sign_prot_Hi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_100uM_stim_sign_proteins.txt")
write(sign_prot_iSYK_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iSYK_sign_proteins.txt")
write(sign_prot_iBTK_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iBTK_sign_proteins.txt")
write(sign_prot_iPI3Kd_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iPI3Kd_sign_proteins.txt")
write(sign_prot_iNFkB_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iNFkB_sign_proteins.txt")

write(sign_prot_iSYK_Hcs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iSYK_stim_sign_proteins.txt")
write(sign_prot_iBTK_Hcs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iBTK_stim_sign_proteins.txt")
write(sign_prot_iNFkB_Hcs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iNFkB_stim_sign_proteins.txt")

# Dataset with significantly changing proteins, and log2FC + pvalues from DESeq2 analysis: clean_data_sign
write.csv(clean_data_sign_Hi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_100uM_stim_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iSYK_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iSYK_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iBTK_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iBTK_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iPI3Kd_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iPI3Kd_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iNFkB_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iNFkB_sign_DESeq2data.csv", row.names = F)

# Dataset of counts per protein per sample + heatmap data for significantly changing proteins: clean_data_well
write.csv(clean_data_well_Hi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_100uM_stim_sign_sample.csv", row.names = F)
write.csv(clean_data_well_inhib_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_inhib_sign_sample.csv", row.names = F)

# Dataset of counts per protein per condition (mean of repl) for significantly changing proteins: clean_data_mean
write.csv(clean_data_mean_Hi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_100uM_stim_sign_condition.csv", row.names = F)
write.csv(clean_data_mean_Hs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_inhib_sign_condition.csv", row.names = F)
```
...
cts <- counts_dtbl  %>%
  dplyr::filter(cell_line == "OCI-Ly8" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
```{r fig_heatmap_act_Ly8_int_100, fig.width=21, fig.height=7}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)
...
```{r fig_heatmap_act_Ly8_int_100s, fig.width=15, fig.height=5.5}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)
...
```{r log2FC_act_Ly8_conc}
# Create results for all DESeq_comparisons
results_iSYK0.1_Ocs <- results(dds, name = "inhib_text_new0.1.uM.iSYK.stimulusaIg.H2O2", alpha = 0.05)
results_iSYK1_Ocs <- results(dds, name = "inhib_text_new1.uM.iSYK.stimulusaIg.H2O2", alpha = 0.05)
results_iSYK10_Ocs <- results(dds, name = "inhib_text_new10.uM.iSYK.stimulusaIg.H2O2", alpha = 0.05)
results_iSYK100_Ocs <- results(dds, name = "inhib_text_new100.uM.iSYK.stimulusaIg.H2O2", alpha = 0.05)

results_iBTK0.1_Ocs <- results(dds, name = "inhib_text_new0.1.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK1_Ocs <- results(dds, name = "inhib_text_new1.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK10_Ocs <- results(dds, name = "inhib_text_new10.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)
results_iBTK100_Ocs <- results(dds, name = "inhib_text_new100.uM.iBTK.stimulusaIg.H2O2", alpha = 0.05)

results_iPI3Kd0.1_Ocs <- results(dds, name = "inhib_text_new0.1.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd1_Ocs <- results(dds, name = "inhib_text_new1.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd10_Ocs <- results(dds, name = "inhib_text_new10.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)
results_iPI3Kd100_Ocs <- results(dds, name = "inhib_text_new100.uM.iPI3Kd.stimulusaIg.H2O2", alpha = 0.05)

results_iNFkB0.1_Ocs <- results(dds, name = "inhib_text_new0.1.uM.iNFkB.stimulusaIg.H2O2", alpha = 0.05)
results_iNFkB1_Ocs <- results(dds, name = "inhib_text_new1.uM.iNFkB.stimulusaIg.H2O2", alpha = 0.05)
results_iNFkB10_Ocs <- results(dds, name = "inhib_text_new10.uM.iNFkB.stimulusaIg.H2O2", alpha = 0.05)
results_iNFkB100_Ocs <- results(dds, name = "inhib_text_new100.uM.iNFkB.stimulusaIg.H2O2", alpha = 0.05)

# results_CD53_aIg <- results(dds, contrast = c("description_cell_stim", "CD53_KO anti-Ig", "WT_ctrl anti-Ig"), alpha = 0.05)

list_results <- list(results_iSYK0.1_Ocs, results_iSYK1_Ocs, results_iSYK10_Ocs, results_iSYK100_Ocs, 
                     results_iBTK0.1_Ocs, results_iBTK1_Ocs, results_iBTK10_Ocs, results_iBTK100_Ocs, 
                     results_iPI3Kd0.1_Ocs, results_iPI3Kd1_Ocs, results_iPI3Kd10_Ocs, results_iPI3Kd100_Ocs, 
                     results_iNFkB0.1_Ocs, results_iNFkB1_Ocs, results_iNFkB10_Ocs, results_iNFkB100_Ocs)

# Results summary
summary(results_iSYK100_Ocs, padj = 0.05)
# sum(results_aIg$padj < 0.05, na.rm = TRUE)
```

```{r fig_log2FC_act_Ly8_conc, fig.width=7, fig.height=4}
par(mfrow = c(1,3), mar = c(4,4,2,1))
# par(mfrow = c(1,1))
signcutoff <- 0.05
plotMA(results_iBTK1_Ocs, alpha = signcutoff, main = "Effect of 1 uM iBTK on\n aIg+H2O2 vs PBS")
plotMA(results_iBTK10_Ocs, alpha = signcutoff, main = "Effect of 10 uM iBTK on\n aIg+H2O2 vs PBS")
plotMA(results_iBTK100_Ocs, alpha = signcutoff, main = "Effect of 100 uM iBTK on\n aIg+H2O2 vs PBS")
```


##### Data transformation {.tabset}

```{r transformation_act_Ly8_conc}
# Normtransform (log2(n+1))
ntd <- normTransform(dds)
...
```{r fig_PCA_act_Ly8_conc, fig.width=10, fig.height=4}
list_datatransform <- list(ntd, vsd, rld)
titles_datatransform <- c("ntd", "vsd", "rld")
...
```{r fig_sd_act_Ly8_conc, fig.width=6, fig.height=3}
options(repr.plot.width = 6, repr.plot.height = 3)
...
```{r fig_distances_act_Ly8_conc, fig.width=10, fig.height=15}
options(repr.plot.width = 10, repr.plot.height = 15)
...
# Apply function to each dataset
names_results_volc <- c("volc_iSYK0.1_Ocs", "volc_iSYK1_Ocs", "volc_iSYK10_Ocs", "volc_iSYK100_Ocs", 
                        "volc_iBTK0.1_Ocs", "volc_iBTK1_Ocs", "volc_iBTK10_Ocs", "volc_iBTK100_Ocs", 
                        "volc_iPI3Kd0.1_Ocs", "volc_iPI3Kd1_Ocs", "volc_iPI3Kd10_Ocs", "volc_iPI3Kd100_Ocs", 
                        "volc_iNFkB0.1_Ocs", "volc_iNFkB1_Ocs", "volc_iNFkB10_Ocs", "volc_iNFkB100_Ocs")
...
```{r fig_volcano_act_Ly8_conc, fig.width=8, fig.height=6.5}
# Function to create volcano plots
# Standard log2FC threshold = log2(1.5); standard padj threshold = 0.05
vulcanoplot_results <- function(data = data, 
                                title_plot = "", 
...
# Apply function to each dataset
list_results_volc <- list(volc_iSYK0.1_Ocs, volc_iSYK1_Ocs, volc_iSYK10_Ocs, volc_iSYK100_Ocs, 
                        volc_iBTK0.1_Ocs, volc_iBTK1_Ocs, volc_iBTK10_Ocs, volc_iBTK100_Ocs, 
                        volc_iPI3Kd0.1_Ocs, volc_iPI3Kd1_Ocs, volc_iPI3Kd10_Ocs, volc_iPI3Kd100_Ocs, 
                        volc_iNFkB0.1_Ocs, volc_iNFkB1_Ocs, volc_iNFkB10_Ocs, volc_iNFkB100_Ocs)
titles_volcano <- c("Effect of 0.1 uM iSYK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 1 uM iSYK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 10 uM iSYK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 100 uM iSYK on aIg+H2O2 vs PBS (OCI-Ly8)", 
                    "Effect of 0.1 uM iBTK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 1 uM iBTK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 10 uM iBTK on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 100 uM iBTK on aIg+H2O2 vs PBS (OCI-Ly8)", 
                    "Effect of 0.1 uM iPI3Kd on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 1 uM iPI3Kd on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 10 uM iPI3Kd on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 100 uM iPI3Kd on aIg+H2O2 vs PBS (OCI-Ly8)", 
                    "Effect of 0.1 uM iNFkB on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 1 uM iNFkB on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 10 uM iNFkB on aIg+H2O2 vs PBS (OCI-Ly8)", "Effect of 100 uM iNFkB on aIg+H2O2 vs PBS (OCI-Ly8)")
...
***Figure:** Volcano plots of DESeq2 model comparisons: the effect of inhibitor conc on aIg+H2O2 vs PBS (OCI-Ly8 cells only). Higher conc of inhibitor significantly affects more proteins, except iPI3Kd.*
...
# Apply function to each dataset
names_results_heat <- c("heat_iSYK0.1_Ocs", "heat_iSYK1_Ocs", "heat_iSYK10_Ocs", "heat_iSYK100_Ocs", 
                        "heat_iBTK0.1_Ocs", "heat_iBTK1_Ocs", "heat_iBTK10_Ocs", "heat_iBTK100_Ocs", 
                        "heat_iPI3Kd0.1_Ocs", "heat_iPI3Kd1_Ocs", "heat_iPI3Kd10_Ocs", "heat_iPI3Kd100_Ocs", 
                        "heat_iNFkB0.1_Ocs", "heat_iNFkB1_Ocs", "heat_iNFkB10_Ocs", "heat_iNFkB100_Ocs")
...
```{r fig_heatmap_act_Ly8_conc_iSYK, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iSYK effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iSYK_Ocs <- list(heat_iSYK0.1_Ocs, heat_iSYK1_Ocs, heat_iSYK10_Ocs, heat_iSYK100_Ocs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# 100 uM iSYK effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iSYK_Ocs <- t(scale(t(assay(rld)[unique(c(heat_iSYK_Ocs$target_nospace)), rownames(data_heatmap_iSYK)])))
...
ComplexHeatmap::pheatmap(
  ...
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iSYK$inhib_new,
  # row_split = subset(heat_iSYK_Ocs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iSYK conc on aIg+H2O2 vs PBS (OCI-Ly8)"
)
```

```{r fig_heatmap_act_Ly8_conc_iBTK, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iBTK effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iBTK_Ocs <- list(heat_iBTK0.1_Ocs, heat_iBTK1_Ocs, heat_iBTK10_Ocs, heat_iBTK100_Ocs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# 100 uM iBTK effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iBTK_Ocs <- t(scale(t(assay(rld)[unique(c(heat_iBTK_Ocs$target_nospace)), rownames(data_heatmap_iBTK)])))
...
ComplexHeatmap::pheatmap(
  ...
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iBTK$inhib_new,
  # row_split = subset(heat_iBTK_Ocs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iBTK conc on aIg+H2O2 vs PBS (OCI-Ly8)"
)
```

```{r fig_heatmap_act_Ly8_conc_iPI3Kd, fig.width=8, fig.height=4, eval=F}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iPI3Kd effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iPI3Kd_Ocs <- list(heat_iPI3Kd0.1_Ocs, heat_iPI3Kd1_Ocs, heat_iPI3Kd10_Ocs, heat_iPI3Kd100_Ocs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# 100 uM iPI3Kd effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iPI3Kd_Ocs <- t(scale(t(assay(rld)[unique(c(heat_iPI3Kd_Ocs$target_nospace)), rownames(data_heatmap_iPI3Kd)])))
...
as.ggplot(ComplexHeatmap::pheatmap(
  ...
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iPI3Kd$inhib_new,
  # row_split = subset(heat_iPI3Kd_Ocs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  ...
  labs(title = "Effect of iPI3Kd conc on aIg+H2O2 vs PBS (OCI-Ly8)")
```

```{r fig_heatmap_act_Ly8_conc_iNFkB, fig.width=8, fig.height=4}
# , fig.width=10, fig.height=10
# options(repr.plot.width = 10, repr.plot.height = 10)

# Combine all heatmap datasets of iNFkB effect on aIg+H2o2 vs PBS (without DESeq2 analysis values)
heat_iNFkB_Ocs <- list(heat_iNFkB0.1_Ocs, heat_iNFkB1_Ocs, heat_iNFkB10_Ocs, heat_iNFkB100_Ocs) %>% 
  purrr::reduce(full_join) %>%
  dplyr::select(-c(baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express)) %>%
  dplyr::filter(target_nospace != "BIM") %>%
  distinct()
...
# 100 uM iNFkB effect on aIg+H2O2 vs PBS
# Get the rld transformed data for the heatmap and scale per row
data_iNFkB_Ocs <- t(scale(t(assay(rld)[unique(c(heat_iNFkB_Ocs$target_nospace)), rownames(data_heatmap_iNFkB)])))
...
ComplexHeatmap::pheatmap(
  ...
  colorRampPalette(c("navy", "white", "firebrick3"))(50),
  column_split = data_heatmap_iNFkB$inhib_new,
  # row_split = subset(heat_iNFkB_Ocs)$modification,
  cellwidth = 8,
  cellheight = 8,
  fontsize = 8,
  border_color = NA,
  main = "Effect of iNFkB conc on aIg+H2O2 vs PBS (OCI-Ly8)"
)
```
...
cts <- counts_dtbl  %>%
  dplyr::filter(cell_line == "OCI-Ly8" & stimulus == "aIg+H2O2" & true_counts >= 1) %>%
  dplyr::select(target_nospace, plate_well, true_counts) %>%
  dplyr::filter(!is.na(true_counts)) %>%
  spread(plate_well, true_counts) %>%
  replace(is.na(.), 0)
...
Finally, we visualised the normalised true counts of all proteins for which the aIg+H2O2 vs PBS comparison was significantly affected by 100 uM inhibitor (19 in total).
...
sign_prot_iSYK_Ocs <- sort(unique(heat_iSYK_Ocs$target_nospace))
sign_prot_iBTK_Ocs <- sort(unique(heat_iBTK_Ocs$target_nospace))
sign_prot_iNFkB_Ocs <- sort(unique(heat_iNFkB_Ocs$target_nospace))
...
for (heat_data in c(1:length(heat_list_Oi))) {
  ...
  alt_data <- heat_list_Oi[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_Oi) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iSYK_Os))) {
  ...
  alt_data <- heat_list_iSYK_Os[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iSYK_Os) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iBTK_Os))) {
  ...
  alt_data <- heat_list_iBTK_Os[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iBTK_Os) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iPI3Kd_Os))) {
  ...
  alt_data <- heat_list_iPI3Kd_Os[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iPI3Kd_Os) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
for (heat_data in c(1:length(heat_list_iNFkB_Os))) {
  ...
  alt_data <- heat_list_iNFkB_Os[[heat_data]] %>%
    dplyr::filter(target_nospace %in% sign_prot_iNFkB_Os) %>%
    dplyr::select(target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, isotype, barcode_name, DS_Ab_nr, 
           baseMean, log2FoldChange, lfcSE, stat, pvalue, padj, diff_express) %>% # reorder cols to more logical order
    dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
                  marker_path_detail = marker_pathway_detail, 
...
# Effect of inhibitor conc on activated signaling
data_iSYK_Os <- t(scale(t(assay(rld)[unique(c(sign_prot_iSYK_Os)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iSYK")))])))
...
data_iBTK_Os <- t(scale(t(assay(rld)[unique(c(sign_prot_iBTK_Os)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iBTK")))])))
...
data_iPI3Kd_Os <- t(scale(t(assay(rld)[unique(c(sign_prot_iPI3Kd_Os)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iPI3Kd")))])))
...
data_iNFkB_Os <- t(scale(t(assay(rld)[unique(c(sign_prot_iNFkB_Os)), rownames(dplyr::filter(data_heatmap, inhib_new %in% c("PBS ctrl", "DMSO ctrl", "iNFkB")))])))
...
clean_data_well_inhib_Os <- list(alt_data_iSYK_Os, alt_data_iBTK_Os, alt_data_iPI3Kd_Os, alt_data_iNFkB_Os) %>% 
  ...
  left_join(counts_dtbl) %>%
  arrange(plate_well, target_nospace) %>%
  dplyr::filter(target_nospace %in% sign_prot_inhib_Os) %>%
  dplyr::select(plate_well, plate, well, experiment, cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, replicate, description, descript_rep,
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         true_counts, true_scaling_factor_geo, true_counts_norm_geo, counts_norm_0, man_FC_norm, man_log2FC_norm, heatmap_iSYK, heatmap_iBTK, heatmap_iPI3Kd, heatmap_iNFkB) %>% # reorder columns for a more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_mean_Oi <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_Oi & cell_line == "OCI-Ly8") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
clean_data_mean_Os <- mean_dtbl %>%
  dplyr::filter(target_nospace %in% sign_prot_inhib_Os & cell_line == "OCI-Ly8") %>%
  dplyr::select(cell_line, stimulus, inhibitor, inhib_conc_uM, inhib_conc, inhib_conc_text, description, 
         target_nospace, target, modification, marker_cell, marker_pathway, marker_pathway_detail, barcode_name, DS_Ab_nr, 
         counts, counts_sd, counts_norm, counts_norm_sd, FC_norm, FC_norm_sd, log2FC_norm, log2FC_norm_sd) %>% # reorder cols to more logical order
  dplyr::rename(marker_path = marker_pathway, # rename some columns for clarity
...
```{r fig_counts_act_Ly8_all, fig.width=15, fig.height=12}
...
Effect of inhibitor concentration on aIg+H2O2 vs PBS comparison (OCI-Ly8 cells only):
For each inhibitor (iSYK, iBTK, iPI3Kd, iNFkB):

- Ly8_inhib_stim_sign_proteins.txt: List of significantly affected proteins by any inhibitor concentration in the aIg+H2O2 vs PBS comparison
...
```{r save_act_Ly8}
# List of significantly changing proteins: sign_prot_cell
write(sign_prot_Oi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_100uM_stim_sign_proteins.txt")
write(sign_prot_iSYK_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iSYK_sign_proteins.txt")
write(sign_prot_iBTK_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iBTK_sign_proteins.txt")
write(sign_prot_iPI3Kd_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iPI3Kd_sign_proteins.txt")
write(sign_prot_iNFkB_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iNFkB_sign_proteins.txt")

write(sign_prot_iSYK_Ocs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iSYK_stim_sign_proteins.txt")
write(sign_prot_iBTK_Ocs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iBTK_stim_sign_proteins.txt")
write(sign_prot_iNFkB_Ocs, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iNFkB_stim_sign_proteins.txt")

# Dataset with significantly changing proteins, and log2FC + pvalues from DESeq2 analysis: clean_data_sign
write.csv(clean_data_sign_Oi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_100uM_stim_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iSYK_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iSYK_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iBTK_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iBTK_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iPI3Kd_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iPI3Kd_sign_DESeq2data.csv", row.names = F)
write.csv(clean_data_sign_iNFkB_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iNFkB_sign_DESeq2data.csv", row.names = F)

# Dataset of counts per protein per sample + heatmap data for significantly changing proteins: clean_data_well
write.csv(clean_data_well_Oi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_100uM_stim_sign_sample.csv", row.names = F)
write.csv(clean_data_well_inhib_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_inhib_sign_sample.csv", row.names = F)

# Dataset of counts per protein per condition (mean of repl) for significantly changing proteins: clean_data_mean
write.csv(clean_data_mean_Oi, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_100uM_stim_sign_condition.csv", row.names = F)
write.csv(clean_data_mean_Os, file = "output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_inhib_sign_condition.csv", row.names = F)
```
...
### Single protein plots

-   Dot plots of all samples

-   Bar plots of log2FC of aIg+H2O2 w/ inhib conc vs PBS w/ same inhib conc (instead of vs PBS w/ DMSO ctrl)

```{r fig_dot_bar_all, fig.width=12, fig.height=5}
options(repr.plot.width = 5, repr.plot.height = 5)
...
for(protein in all_prot) {
  ...
  plot2 <- ggplot(subset(data_mean_all, stimulus == "aIg+H2O2" & target_nospace == protein)) +
    geom_col(aes(
      x = factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label),
      y = man_log2FC_norm, 
      ...
      fill = inhib_conc),
      position = "dodge2") + 
    geom_errorbar(data = subset(data_mean_all, stimulus == "aIg+H2O2" & target_nospace == protein),
                  aes(x = factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label),
                      ymin = man_log2FC_norm - man_log2FC_norm_sd,
                      ymax = man_log2FC_norm + man_log2FC_norm_sd,
                      group = stimulus
                  ),
                  width = 0.75, size = 0.75, color = "#525252", position = "dodge2") +
    facet_grid(vars(factor(cell_line, levels = cells_DS108)), vars(factor(inhibitor, levels = inhib_DS108))) +
    scale_fill_manual(values = colors_blue9[c(2, 3, 5, 6, 8, 9)], name = "Inhib conc") +
    labs(x = "", y = "Manual log2FC (norm)", title = protein) +
    theme_bw() +
    theme(legend.position = "none", panel.grid.major.x = element_blank()) +
    textsize_small
  print(plot2)
  ...
  plot4 <- ggplot() +
    geom_point(data = subset(data_well_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
                aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label), 
                    counts_norm, 
                ...
                alpha = 1, size = 1.25) +
    geom_line(data = subset(data_mean_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
                  aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label),
                      counts_norm,
    ...
    facet_wrap(vars(factor(inhibitor, levels = inhib_DS108))) +
    # scale_y_log10() +
    scale_color_manual(values = c("HBL1 PBS" = "#c994c7", "HBL1 aIg+H2O2" = "#e7298a", "OCI-Ly8 PBS" = "#7fcdbb", "OCI-Ly8 aIg+H2O2" = "#1d91c0"), name = "Cell line + stimulus") +
    labs(x = "Inhibitor concentration", y = "Normalized counts", title = protein) +
    theme_bw() +
    # theme(legend.position = "none", panel.grid.major.x = element_blank()) +
    textsize_small
  print(plot4)
...
Bar plot 'fingerprints' for each condition

```{r fig_print, fig.width=18, fig.height=5}
# TEST: cell line comparison w/o stim w/o inhib
test_data <- data_mean_all %>%
  # dplyr::dplyr::filter(description == "HBL1 - 0 uM iBTK (DMSO) - aIg+H2O2")
  dplyr::filter(stimulus == "PBS" & inhib_conc == "0 uM (PBS)") %>%
  dplyr::group_by(cell_line, inhib_conc, stimulus, target_nospace) %>%
  summarise(mean_counts_norm_sd = sd(counts_norm_sd, na.rm = T), 
            mean_counts_norm = mean(counts_norm, na.rm = T))
...
plot <- ggplot(test_data) +
  geom_col(aes(
    x = target_nospace,
    y = mean_counts_norm,
    group = factor(cell_line, levels = cells_DS108),
    fill = factor(cell_line, levels = cells_DS108)),
    alpha = 0.5,
    position = "identity") +
  geom_errorbar(aes(
    x = target_nospace,
    ...
    group = cell_line),
    width = 0.75, size = 0.75, color = "#525252", position = "identity") +
  scale_y_log10(
    breaks = trans_breaks("log10", function(x) 10^x),
    labels = trans_format("log10", math_format(10^.x)), 
    expand = expansion(mult = c(0, 0.05))) +
  annotation_logticks(sides = "l") +
  scale_y_break(breaks = c(0.2e1, 3e2)) +
  # coord_cartesian(ylim = c(1e2, 1e4)) +
  scale_fill_manual(values = colors_cell, name = "Cell line") +
  # scale_fill_manual(values = c("#cb181d", "#238b45"), name = "Cell line") +
  # facet_wrap(~description) +
  labs(x = "Protein", y = "Normalised compensated counts") +
  theme_bw() +
  RotatedAxis() +
...
```{r fig_print_all, fig.width=26, fig.height=26}
plot <- ggplot(subset(data_mean_all, inhib_conc != "0 uM (PBS)" & inhibitor != "iPI3Kd" & modification != "no")) +
  geom_col(aes(
    x = target_nospace,
    y = counts_norm,
    group = factor(stimulus, levels = stim_DS108),
    fill = factor(stimulus, levels = stim_DS108)),
    alpha = 0.5,
    position = "identity") +
  geom_errorbar(aes(
    x = target_nospace,
  ...
  scale_y_log10(
    ...
    expand = expansion(mult = c(0, 0.05))) +
  annotation_logticks(sides = "l") +
  # scale_y_break(breaks = c(0.2e1, 1e2)) +
  coord_cartesian(ylim = c(2.5e2, 1e4)) +
  scale_fill_manual(values = colors_stim, name = "Stimulus") +
  facet_grid(vars(factor(inhib_conc_text)), vars(factor(cell_line, level = cells_DS108))) +
  # facet_wrap(vars(factor(inhib_conc_text), factor(cell_line, level = cells_DS108)), ncol = 2) +
  labs(x = "Protein", y = "Normalised compensated counts") +
  theme_bw() +
  RotatedAxis() +
  theme(panel.grid.major.x = element_blank()) +
...
```{r fig_print_inhib_HBL1, fig.width=18, fig.height=18}
# HBL1 w/ aIg+H2O2 samples, per inhib conc
plot <- ggplot(subset(data_mean_all, inhib_conc != "0 uM (PBS)" & inhibitor != "iPI3Kd" & modification != "no" & cell_line == "HBL1" & stimulus == "aIg+H2O2")) + #inhib_conc == "100 uM" & 
  geom_col(aes(
    x = target_nospace,
    y = counts_norm,
    # group = factor(stimulus, levels = stim_DS108),
  ...
  # scale_y_break(breaks = c(0.2e1, 1e2)) +
  coord_cartesian(ylim = c(2.5e2, 1e4)) +
  # scale_fill_manual(values = colors_inhib, name = "Inhibitor") +
  # facet_grid(vars(factor(inhib_conc_text)), vars(factor(cell_line, level = cells_DS108))) +
  # facet_wrap(vars(factor(inhib_conc_text), factor(cell_line, level = cells_DS108)), ncol = 2) +
  facet_wrap(~inhib_conc, ncol = 1) + 
  labs(x = "Protein", y = "Normalised compensated counts", title = "HBL1 w/ aIg+H2O2") +
  theme_bw() +
  RotatedAxis() +
  theme(panel.grid.major.x = element_blank()) +
  textsize_small
...
# HBL1 w/ aIg+H2O2 samples, per inhibitor
plot <- ggplot(subset(data_mean_all, inhib_conc != "0 uM (PBS)" & inhibitor != "iPI3Kd" & modification != "no" & cell_line == "HBL1" & stimulus == "aIg+H2O2")) + #inhib_conc == "100 uM" & 
  geom_col(aes(
    x = target_nospace,
    y = counts_norm,
  ...
  # scale_y_break(breaks = c(0.2e1, 1e2)) +
  coord_cartesian(ylim = c(2.5e2, 1e4)) +
  scale_fill_manual(values = colors_blue9[c(2, 3, 5, 7, 9)], name = "Inhib conc") +
  # facet_grid(vars(factor(inhib_conc_text)), vars(factor(cell_line, level = cells_DS108))) +
  # facet_wrap(vars(factor(inhib_conc_text), factor(cell_line, level = cells_DS108)), ncol = 2) +
...
```{r fig_print_inhib_Ly8, fig.width=18, fig.height=18}
# HBL1 w/ aIg+H2O2 samples, per inhib conc
plot <- ggplot(subset(data_mean_all, inhib_conc != "0 uM (PBS)" & inhibitor != "iPI3Kd" & modification != "no" & cell_line == "OCI-Ly8" & stimulus == "aIg+H2O2")) + #inhib_conc == "100 uM" & 
  geom_col(aes(
    x = target_nospace,
    y = counts_norm,
    # group = factor(stimulus, levels = stim_DS108),
  ...
  annotation_logticks(sides = "l") +
  # scale_y_break(breaks = c(0.2e1, 1e2)) +
  coord_cartesian(ylim = c(2.5e2, 1.1e4)) +
  # scale_fill_manual(values = c("red", "cyan", "yellow"), name = "Inhibitor") +
  # facet_grid(vars(factor(inhib_conc_text)), vars(factor(cell_line, level = cells_DS108))) +
  # facet_wrap(vars(factor(inhib_conc_text), factor(cell_line, level = cells_DS108)), ncol = 2) +
  facet_wrap(~inhib_conc, ncol = 1) + 
  labs(x = "Protein", y = "Normalised compensated counts", title = "OCI-Ly8 w/ aIg+H2O2") +
  theme_bw() +
  RotatedAxis() +
  theme(panel.grid.major.x = element_blank()) +
  textsize_small
...
### Paper fig tests {.tabset}

#### Fig 1
```{r paper_test_fig1, fig.width=7, fig.height=3.5}
all_prot <- sort(unique(c(data_well_all$target_nospace)))
prot_test1 <- c("CD79a_Y182", "SYK_Y525_Y526")

for(protein in prot_test1) {
  plot1 <- ggplot() +
    geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & target_nospace == protein & inhib_conc %in% c("0 uM (DMSO)", "100 uM")), 
                # aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label), 
                aes(paste(cell_line, "\n", inhib_conc), 
                    counts_norm, 
                    color = stimulus, 
                ...
                alpha = 1, size = 1.25, width = 0.1) +
    geom_errorbar(data = subset(data_mean_all, inhibitor != "iPI3Kd" & target_nospace == protein & inhib_conc %in% c("0 uM (DMSO)", "100 uM")),
                  # aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label),
                  aes(paste(cell_line, "\n", inhib_conc),
                      ymin = counts_norm,
                      ymax = counts_norm,
                  ...
                  width = 0.5, size = 0.75) + #, color = "black"
    facet_wrap(vars(factor(inhibitor, levels = inhib_DS108)), ncol = 3) + #vars(factor(cell_line, levels = cells_DS108)),
    # scale_y_log10() +
    scale_color_manual(values = colors_stim, name = "Stimulus") + 
    labs(x = "", y = "Normalized counts", title = protein) +
    theme_bw() +
...
```{r paper_test_fig2_a, fig.width=4, fig.height=3}
data_properties_true$step <- "Comp"
data_properties_norm_true$step <- "Norm"
data_properties_norm_true2 <- dplyr::rename(data_properties_norm_true, nCount = nCount_norm)
data_test2 <- full_join(data_properties_true, data_properties_norm_true2) %>%
subset(staining == "yes")

fig_test2 <- ggplot(data_test2, aes(cell_line, nCount, color = inhib_conc, shape = stimulus)) +
  geom_jitter(alpha = 1, size = 1, width = 0.1) +
  facet_wrap(vars(factor(step, levels = c("Comp", "Norm"), labels = c("Raw counts", "Normalised counts")))) +
  theme_bw() +
  # RotatedAxis() +
  scale_y_log10() +
  # scale_color_manual(values = c("PBS" = "black", "aIg+H2O2" = "orange"), name = "") +
  scale_color_manual(values = colors_blue9[c(3:8)]) +
  labs(x = "Inhibitor conc", y = "Counts (log10 scale)") + 
  textsize_small
fig_test2
```

```{r paper_test_fig2_b, fig.width=8, fig.height=3}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))

# LYN
prot_test2 <- c("LYN", "LYN_Y397", "SRC", "SRC_Y416")

filt_mean_test2 <- data_mean_all %>%
  subset(inhibitor != "iPI3Kd" & inhib_conc == "0 uM (PBS)" & target_nospace %in% prot_test2) %>%
  dplyr::group_by(cell_line, stimulus, target_nospace) %>%
  summarise(counts_norm_sd = sd(counts_norm_sd, na.rm = T), 
            counts_norm = mean(counts_norm, na.rm = T))

plot1 <- ggplot() + 
  geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & inhib_conc == "0 uM (PBS)" & target_nospace %in% prot_test2), #target_nospace == protein & 
              aes(cell_line, 
                  counts_norm, 
                  color = stimulus, 
                  # shape = cell_line
              ...
              alpha = 1, size = 1.25, width = 0.1) +
  geom_errorbar(data = filt_mean_test2,
                aes(cell_line,
                    ymin = counts_norm,
                    ymax = counts_norm,
                    group = stimulus,
                ...
                width = 0.5, size = 0.75) + #, color = "black"
  facet_wrap(vars(factor(target_nospace, levels = prot_test2)), ncol = 4) +
  # scale_y_log10() +
  scale_color_manual(values = colors_stim, name = "Stimulus") + 
  labs(x = "", y = "Normalized counts") +
  theme_bw() +
...
```{r paper_test_fig2_c, fig.width=6, fig.height=3}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))

# BTK
prot_test3 <- c("BTK", "BTK_Y223", "BTK_Y551")

filt_mean_test3 <- data_mean_all %>%
  subset(inhibitor != "iPI3Kd" & inhib_conc == "0 uM (PBS)" & target_nospace %in% prot_test3) %>%
  dplyr::group_by(cell_line, stimulus, target_nospace) %>%
  summarise(counts_norm_sd = sd(counts_norm_sd, na.rm = T), 
            counts_norm = mean(counts_norm, na.rm = T))

plot1 <- ggplot() + 
  geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & inhib_conc == "0 uM (PBS)" & target_nospace %in% prot_test3), #target_nospace == protein & 
              aes(cell_line, 
                  counts_norm, 
                  color = stimulus, 
                  # shape = cell_line
              ...
              alpha = 1, size = 1.25, width = 0.1) +
  geom_errorbar(data = filt_mean_test3,
                aes(cell_line,
                    ymin = counts_norm,
                    ymax = counts_norm,
                    group = stimulus,
                ...
                width = 0.5, size = 0.75) + #, color = "black"
  facet_wrap(vars(factor(target_nospace, levels = prot_test3)), ncol = 4) +
  # scale_y_log10() +
  scale_color_manual(values = colors_stim, name = "Stimulus") + 
  labs(x = "", y = "Normalized counts") +
  theme_bw() +
...
```{r paper_test_fig2_d, fig.width=4, fig.height=3}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))
# Isotypes
prot_test4 <- c("Isotype_Mouse_IgG1", "Isotype_Rabbit_IgG")

filt_mean_test4 <- data_mean_all %>%
  subset(inhibitor != "iPI3Kd" & target_nospace %in% prot_test4 & inhib_conc %in% c("0 uM (DMSO)", "100 uM")) %>%
  dplyr::group_by(cell_line, stimulus, inhib_conc, target_nospace) %>%
  summarise(counts_norm_sd = sd(counts_norm_sd, na.rm = T), 
            counts_norm = mean(counts_norm, na.rm = T))

plot1 <- ggplot() + 
  geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & target_nospace %in% prot_test4 & inhib_conc %in% c("0 uM (DMSO)", "100 uM")), #& inhib_conc == "0 uM (PBS)" & 
              aes(paste(cell_line, "\n", inhib_conc), 
                  counts_norm, 
                  color = stimulus, 
                  # shape = cell_line
              ...
              alpha = 1, size = 1.25, width = 0.1) +
  geom_errorbar(data = filt_mean_test4,
                aes(paste(cell_line, "\n", inhib_conc),
                    ymin = counts_norm,
                    ymax = counts_norm,
                    group = stimulus,
...
```{r paper_test_fig3_a}
# Protein lists
prot_iSYK_H <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iSYK_stim_sign_proteins.txt")
prot_iBTK_H <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iBTK_stim_sign_proteins.txt")
prot_iNFkB_H <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/HBL1_iNFkB_stim_sign_proteins.txt")

prot_iSYK_O <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iSYK_stim_sign_proteins.txt")
prot_iBTK_O <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iBTK_stim_sign_proteins.txt")
prot_iNFkB_O <- read_lines("output/DS108_StimInhibIDseq/IDseq_sign_threshold/active_inhibitor/Ly8_iNFkB_stim_sign_proteins.txt")

# HBL1
prot_data_H <- list(prot_iSYK_H, prot_iBTK_H, prot_iNFkB_H)
prot_names_H <- c(paste0("iSYK (", length(prot_iSYK_H), " total)"), 
                  paste0("iBTK (", length(prot_iBTK_H), " total)"), 
                  paste0("iNFkB (", length(prot_iNFkB_H), " total)"))
names(prot_data_H) <- prot_names_H
plot_H <- ggvenn(data = prot_data_H, 
               fill_color = c("#41ab5d", "#4292c6", "#f16913"), 
               # auto_scale = TRUE,
...
# OCI-Ly8
prot_data_O <- list(prot_iSYK_O, prot_iBTK_O, prot_iNFkB_O)
prot_names_O <- c(paste0("iSYK (", length(prot_iSYK_O), " total)"), 
                  paste0("iBTK (", length(prot_iBTK_O), " total)"), 
                  paste0("iNFkB (", length(prot_iNFkB_O), " total)"))
names(prot_data_O) <- prot_names_O
plot_O <- ggvenn(data = prot_data_O, 
               fill_color = c("#41ab5d", "#4292c6", "#f16913"), 
               # auto_scale = TRUE,
               # show_percentage = FALSE,
               # show_elements = TRUE,
...
plot_grid(plot_H, plot_O, ncol = 2)
```

```{r paper_test_fig3_b}
# iSYK
prot_data_iSYK <- list(prot_iSYK_H, prot_iSYK_O)
prot_names_iSYK <- c(paste0("HBL1 (", length(prot_iSYK_H), " total)"), 
                  paste0("OCI-Ly8 (", length(prot_iSYK_O), " total)"))
names(prot_data_iSYK) <- prot_names_iSYK
plot_S <- ggvenn(data = prot_data_iSYK, 
               fill_color = c("#41ab5d", "#4292c6", "#f16913"), 
               auto_scale = TRUE,
               # show_percentage = FALSE,
               # show_elements = TRUE,
...
# iBTK
prot_data_iBTK <- list(prot_iBTK_H, prot_iBTK_O)
prot_names_iBTK <- c(paste0("HBL1 (", length(prot_iBTK_H), " total)"), 
                  paste0("OCI-Ly8 (", length(prot_iBTK_O), " total)"))
names(prot_data_iBTK) <- prot_names_iBTK
plot_B <- ggvenn(data = prot_data_iBTK, 
               fill_color = c("#41ab5d", "#4292c6", "#f16913"), 
               auto_scale = TRUE,
               # show_percentage = FALSE,
               # show_elements = TRUE,
...
# iNFkB
prot_data_iNFkB <- list(prot_iNFkB_H, prot_iNFkB_O)
prot_names_iNFkB <- c(paste0("HBL1 (", length(prot_iNFkB_H), " total)"), 
                  paste0("OCI-Ly8 (", length(prot_iNFkB_O), " total)"))
names(prot_data_iNFkB) <- prot_names_iNFkB
plot_N <- ggvenn(data = prot_data_iNFkB, 
               fill_color = c("#41ab5d", "#4292c6", "#f16913"), 
               auto_scale = TRUE,
               # show_percentage = FALSE,
               # show_elements = TRUE,
...
plot_grid(plot_S, plot_B, plot_N, ncol = 3)
```


#### Fig 4
```{r paper_test_fig4_a, eval=F}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))
prot_test5 <- c("ATF2_T71", "BLNK_Y84", "BTK_Y551", "CREB_S133", 
                "CD79a_Y182", "Caspase3_D175", "PARP_D214", "SYK_Y525_Y526", "ZAP70_Y319", 
                "AMPKa1_2_T183_T172", "IKKa_b_S176_S180", "CD138", "CD79a", "MKK4_S257_T261", "S6_S240_S244", "VAV1_Y174")

for(protein in prot_test5) {
  # Faceted per inhib conc and cell line
  plot1 <- ggplot() +
    geom_jitter(data = subset(data_well_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc %in% c("0 uM (DMSO)", "100 uM")), 
                aes(factor(inhibitor, levels = inhib_DS108), 
                    counts_norm, 
                    color = stimulus, 
                    # shape = cell_line
                    ), 
                alpha = 1, size = 1.25, width = 0.1) +
    geom_errorbar(data = subset(data_mean_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc %in% c("0 uM (DMSO)", "100 uM")), 
                  aes(factor(inhibitor, levels = inhib_DS108),
                      ymin = counts_norm,
                      ymax = counts_norm,
                      group = stimulus,
                  ...
                  width = 0.5, size = 0.75) + #, color = "black"
    facet_grid(vars(factor(cell_line, levels = cells_DS108)), vars(factor(inhib_conc, levels = inhib_conc_DS108))) +
    # scale_y_log10() +
    scale_color_manual(values = colors_stim, name = "Stimulus") + 
    labs(x = "", y = "Normalized counts", title = protein) +
    theme_bw() +
  ...
  # Faceted per inhibitor and cell line
  plot2 <- ggplot() +
    geom_jitter(data = subset(data_well_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
                aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label), 
                    counts_norm, 
                    color = stimulus, 
                    # shape = cell_line
                ...
                alpha = 1, size = 1.25, width = 0.1) +
    geom_errorbar(data = subset(data_mean_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
                  aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label),
                      ymin = counts_norm,
                      ymax = counts_norm,
                      group = stimulus,
...
```{r paper_test_fig4_b, fig.height=4, fig.width=10}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))
prot_test6 <- c("AKT_T308", "PLCy2_Y759", "IKKa_b_S176_S180", # uniquely affected by 1 inhib
                "Histone_H2AX_S139", "ATF2_T71", "MAPK_p38_T180_Y182", # affected by 2 inhib
                "BTK_Y551", "CREB_S133", "MKK4_S257_T261" # affected by all 3 inhib
)

prot_test7 <- c("BTK_Y551", "JAK1_Y1022_Y1023",                    # nice inhib curves in both cell lines with all inhibs
                "CREB_S133", "MAPK_p38_T180_Y182",                 # active + inhib in one cell line
                "ATF2_T71", "S6_S240_S244",                        # inhib in both stim conditions
                "AKT_T308", "cMYC_T58", "STAT6_Y641",              # increase upon inhib
                "IKKa_b_S176_S180", "PARP_D214", "SYK_Y525_Y526"   # increase or decrease depending on inhib
                )

for(protein in prot_test7) {
  # Dotplot faceted per inhibitor and cell line
  plot2 <- ggplot() +
    geom_jitter(data = subset(data_well_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
                aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label), 
                    counts_norm, 
  ...
  # # Barplot faceted per inhibitor
  # plot3 <- ggplot() +
  #   geom_col(data = subset(data_mean_all, target_nospace == protein & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
  #               aes(factor(inhib_conc, levels = inhib_conc_DS108, labels = inhib_conc_label), 
  #                   counts_norm, 
  #                   fill = stimulus,
  #                   group = stimulus
  ...
  #               position = "dodge2") +
  #   # facet_grid(vars(factor(cell_line, levels = cells_DS108)), vars(factor(inhibitor, levels = inhib_DS108))) +
  #   facet_wrap(vars(factor(inhibitor, levels = inhib_DS108), cell_line), nrow = 1) +
  #   # scale_y_log10() +
  #   scale_fill_manual(values = colors_stim, name = "Stimulus") +
  #   labs(x = "", y = "Normalized counts", title = protein) +
  #   theme_bw() +
...
#### Fig 5
```{r paper_test_fig5_a, fig.width=6, fig.height=4}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))
prot_test8 <- c("CD38", "Histone_H2AX_S139", "IgM", "PKCb1_T642", "S6_S235_S236", "VAV1_Y174")

plot1 <- ggplot() +
  geom_jitter(data = subset(data_well_all, target_nospace %in% prot_test8 & inhibitor != "iPI3Kd" & inhib_conc != "0 uM (PBS)"), 
              aes(cell_line, 
                  counts_norm, 
                  color = inhib_conc, 
                  shape = stimulus
  ...
  #                   ),
  #               width = 0.5, size = 0.75) + #, color = "black"
  facet_wrap(~target_nospace) +
  # scale_y_log10() +
  scale_color_manual(values = colors_blue9[4:9]) +
  labs(x = "", y = "Normalized counts") +
  theme_bw() +
  # theme(legend.position = "none", panel.grid.major.x = element_blank()) +
  textsize_small
...
```{r paper_test_fig5_b, fig.width=20, fig.height=20}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))

plot1 <- ggplot() +
  geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & inhib_conc %in% c("0 uM (DMSO)", "100 uM")), 
              aes(inhib_conc, 
                  counts_norm, 
                  color = stimulus, 
                  shape = cell_line
  ...
  #                   ),
  #               width = 0.5, size = 0.75) + #, color = "black"
  facet_wrap(~target_nospace, scales = "free_y") +
  # scale_y_log10() +
  # scale_color_manual(values = colors_blue9[4:9]) +
  scale_color_manual(values = colors_stim, name = "Stimulus") + 
  labs(x = "", y = "Normalized counts") +
...
```{r paper_test_fig5_c, fig.width=8, fig.height=8}
# all_prot <- sort(unique(c(data_well_all$target_nospace)))

# Proteins with reduced variation at 100 uM inhibitor
prot_test9 <- c("BLNK_Y84", "BTK_Y223", "BTK_Y551", "CDK1_T14_Y15", "Cyclin_B1", "JAK1_Y1022_Y1023", "LYN_Y397", "MAPK_p38_T180_Y182", "MKK4_S257_T261", "PYK2_Y402", "SRC_Y416", "VAV1_Y174", "ZAP70_Y319")

filt_mean_test9 <- data_mean_all %>%
  subset(inhibitor != "iPI3Kd" & inhib_conc %in% c("0 uM (DMSO)", "100 uM") & target_nospace %in% prot_test9) %>% 
  dplyr::group_by(cell_line, stimulus, inhib_conc, target_nospace) %>%
  summarise(counts_norm_sd = sd(counts_norm_sd, na.rm = T), 
            counts_norm = mean(counts_norm, na.rm = T))

plot1 <- ggplot() +
  geom_jitter(data = subset(data_well_all, inhibitor != "iPI3Kd" & inhib_conc %in% c("0 uM (DMSO)", "100 uM") & target_nospace %in% prot_test9), 
              aes(inhib_conc, 
                  counts_norm, 
                  color = stimulus, 
                  shape = cell_line
              ...
              alpha = 1, size = 1.25, width = 0.1) +
  geom_errorbar(data = filt_mean_test9, 
                aes(inhib_conc,
                    ymin = counts_norm,
                    ymax = counts_norm,
                    group = stimulus,
                ...
                width = 0.5, size = 0.75) + #, color = "black"
  facet_wrap(~target_nospace, scales = "free_y", ncol = 3) +
  # scale_y_log10() +
  # scale_color_manual(values = colors_blue9[4:9]) +
  scale_color_manual(values = colors_stim, name = "Stimulus") + 
  labs(x = "", y = "Normalized counts") +
